...
def main():
    ...
    val_ds_type = opts_dict['dataset']['val']['type']       # MFQEv2Dataset
    radius = opts_dict['network']['radius']                 # 视频帧序列半径，一次性输入2*radius+1帧，这里是7帧
    ...
    train_ds = train_ds_cls(                                # train_ds = MFQEv2Dataset(opts_dict[...train], radius)
        opts_dict=opts_dict['dataset']['train'], 
        radius=radius
        )
    val_ds = val_ds_cls(                                    # val_ds = MFQEv2Dataset(opts_dict[...val], radius)
    ...
    model = model.to(rank)
    if opts_dict['train']['is_dist']: # 分布式训练
    ...
    for current_epoch in range(start_epoch, num_epoch + 1):
        ...
        while train_data is not None:
            ...
            # get data
            gt_data = train_data['gt'].to(rank)  # (B [RGB] H W) shape :[64,1,128,128]
            lq_data = train_data['lq'].to(rank)  # (B T [RGB] H W) shape :[64,7,1,128,128] 1次输入一个视频序列，7帧
            b, _, c, _, _  = lq_data.shape
            input_data = torch.cat(
                [lq_data[:,:,i,...] for i in range(c)], 
                dim=1
                )  # B [R1 ... R7 G1 ... G7 B1 ... B7] H W shape :[64,7,128,128]
            enhanced_data = model(input_data) # 增强后的data shape :[64,1,128,128]
